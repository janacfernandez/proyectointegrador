window.addEventListener("load", function () {
    ...
    fetch(`https://cors-anywhere.herokuapp.com/https://api.deezer.com/search?q=${busqueda}`)
        ...
        .then(function (datos) {
            ...
            /*Estos console.log me sirven de verificación para saber si están llegando los datos y de que forma llegan.
            console.log(datos);
    ...
    fetch(`https://cors-anywhere.herokuapp.com/https://api.deezer.com/search/playlist?q=${busqueda}`)
        ...
        .then(function (datos) {
            ...
            if (!resultados.length) {
                playlistResults.innerHTML += `<h3>No hay resultados para la playlist buscado</h3>`;
                playlistEspecifico.innerHTML += `<h3>No hay resultados para la playlist buscado</h3>`;
            }else if(resultados.length < 6){
                for (let i=0; i<resultados.length; i++){
                    let playlistFoto = resultados[i].picture_medium;
                    let playlistId = resultados[i].id;
                    let playlist = resultados[i].title;
                    listPlaylists.innerHTML += `<li>
            ...
            <div class="textos">
                ...
                playlistArticle.innerHTML += `<li>
                <figure><img src="${playlistFoto}" alt="${playlist}"></figure>
                <div class="textos">
                    <h3> <a href="detail-playlist.html?id=${playlistId}" title="ver ${playlist}"> ${playlist}</a></h3>
                    <p><a href="detail-playlist.html?id=${playlistId}" title="ver ${playlist}"><i class="fas fa-arrow-right"></i></a>
                </div>
                </li>`;
                }
            }else {
